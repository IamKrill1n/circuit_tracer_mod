...
from summarization.prune import PruneGraph, filter_act_density
from summarization.utils import _node_from_json_dict


def _feature_info(act_density: float) -> FeatureInfo:
    return FeatureInfo(
...
def test_filter_act_density_drops_out_of_band(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_fetch(scan, node_id, **kwargs):
        index = int(node_id.split("_")[1])
        return _feature_info(act_density=0.01 if index == 10 else 0.5)

    monkeypatch.setattr("summarization.prune.fetch_feature_info", fake_fetch)
    ...
    assert "1_20_0" not in ids
    assert "0_10_0" in ids
    assert {"E_0_0", "L_1"}.issubset(ids)

    by_id = {n.node_id: n for n in out.nodes}
    assert by_id["0_10_0"].clerp == ""
    assert by_id["E_0_0"].clerp == "Emb: hi"
    assert out.pruned_adj.shape[0] == len(out.nodes)


def test_filter_act_density_requires_scan() -> None:
