...
def test_pipeline_rejects_conflicting_modes_before_loading(monkeypatch) -> None:
    ...
    args = build_parser().parse_args(
        ["--claim", "claim", "--selector-model", "selector", "--token-weights", "[1]"]
    )
    with pytest.raises(ValueError, match="mutually exclusive"):
        run_pipeline(args)


def test_pipeline_validates_seed_selector_modes_before_loading() -> None:
    from summarization.__main__ import build_parser
    from summarization.pipeline import run_pipeline

    parser = build_parser()
    with pytest.raises(ValueError, match="not used with"):
        run_pipeline(
            parser.parse_args(
                ["--claim", "claim", "--seed-selector", "jev", "--selector-model", "selector"]
            )
        )
    with pytest.raises(ValueError, match="Supply both"):
        run_pipeline(parser.parse_args(["--claim", "claim"]))
    with pytest.raises(ValueError, match="requires --claim"):
        run_pipeline(parser.parse_args(["--seed-selector", "jev"]))
    with pytest.raises(ValueError, match="Supply both"):
        run_pipeline(parser.parse_args(["--selector-model", "selector"]))
    with pytest.raises(ValueError, match="mutually exclusive"):
        run_pipeline(
            parser.parse_args(
                ["--claim", "claim", "--seed-selector", "jev", "--token-weights", "[1]"]
            )
        )
