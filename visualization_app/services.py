...
def list_graphs(root: Path = GRAPH_ROOT) -> list[GraphRecord]:
    if not root.exists():
        return []
    ...
    for directory in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        records.append(
            GraphRecord(
                ...
                has_pt=(directory / f"{slug}.pt").exists(),
                has_summary=(directory / f"{slug}.sng.pt").exists(),
...
def load_graph_record(slug: str, root: Path = GRAPH_ROOT) -> GraphRecord:
    safe_slug = slugify(slug)
    for record in list_graphs(root):
        if record.slug == safe_slug:
            return record
    raise FileNotFoundError(f"Unknown graph slug: {safe_slug}")


def sidecar_pt_path(slug: str, root: Path = GRAPH_ROOT) -> Path:
    safe_slug = slugify(slug)
    return graph_dir(safe_slug, root) / f"{safe_slug}.pt"


def summary_path(slug: str, root: Path = GRAPH_ROOT) -> Path:
    safe_slug = slugify(slug)
    return graph_dir(safe_slug, root) / f"{safe_slug}.sng.pt"
...
def convert_pt_to_viewer(
    ...
    *,
    ...
    destination = sidecar_pt_path(safe_slug, root)
    if pt_path.resolve() != destination.resolve():
        shutil.copy2(pt_path, destination)

    return load_graph_record(safe_slug, root)
...
def generate_graph(
    *,
    ...
    try:
        ...
        pt_path = sidecar_pt_path(safe_slug, root)
...
def run_summary(
    *,
    ...
    if not pt_path.exists():
        raise FileNotFoundError(f"No .pt sidecar exists for graph {safe_slug!r}")

    ag = AttrGraph.from_graph(str(pt_path))
    token_weights = None
    if settings.get("token_weights_source") == "shap":
        import torch

        token_attr_model = str(settings.get("token_attr_model") or settings.get("model_name") or "")
        ...
        token_weights = _token_weights_from_shap(
            ...
            entmax_alpha=(
                float(settings["entmax_alpha"])
                if settings.get("token_attr_normalize") == "entmax"
                else None
            ),
    ...
    if method == "spectral":
        ...
        rows = cluster(
            ...
            ilp_time_limit=float(settings.get("ilp_time_limit", 30.0)),
        )
    else:
    ...
    out = summary_path(safe_slug, root)
    sng.save(str(out))
