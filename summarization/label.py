"""LLM supernode labeling: a model registry, a provider router, and the labeling schemes.
...
def _openai_generate(
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            print(
                f"[label] OpenAI rate limit — retrying in {delay:.0f}s ({attempt + 1}/{_MAX_RETRIES})"
            )
            time.sleep(delay)
            ...
            print(
                f"[label] OpenAI error ({exc.status_code}) — retrying in {delay:.0f}s ({attempt + 1}/{_MAX_RETRIES})"
            )
            time.sleep(delay)
...
def _google_generate(
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            print(
                f"[label] Google GenAI error ({exc}) — retrying in {delay:.0f}s ({attempt + 1}/{_MAX_RETRIES})"
            )
            time.sleep(delay)
