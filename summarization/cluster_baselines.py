...
from summarization.cluster import _cosine_similarity
...
def compute_phi_vectors(
    ...
    n_nodes = adj.shape[0]
    device = adj.device
    inf = _prepare_node_weights(
        prune_graph.node_influence, n_nodes, device, normalize=normalize_weights
    )
    rel = _prepare_node_weights(
        prune_graph.node_relevance, n_nodes, device, normalize=normalize_weights
    )
    sqrt_inf = inf.clamp(min=0.0).sqrt()
    sqrt_rel = rel.clamp(min=0.0).sqrt()
    v_out = adj.T * sqrt_inf.unsqueeze(
        0
    )  # row i = pruned_adj[:, i] (outgoing from i), scaled by sqrt(Inf)
    v_in = adj * sqrt_rel.unsqueeze(
        0
    )  # row i = pruned_adj[i, :] (incoming to i), scaled by sqrt(Rel)
...
def compute_similarity(
    ...
    n_nodes = adj.shape[0]
    node_inf = _prepare_node_weights(
        prune_graph.node_influence, n_nodes, adj.device, normalize=normalize_weights
    )
    node_rel = _prepare_node_weights(
        prune_graph.node_relevance, n_nodes, adj.device, normalize=normalize_weights
    )
...
def cluster_graph_spectral(
    ...
    logger.info(
        "Starting cluster_graph_spectral (target_k=%d, max_layer_span=%s)", target_k, max_layer_span
    )
    ...
    while len(active) > target_k:
        ...
        for ii in range(len(active_list)):
            ...
            for jj in range(ii + 1, len(active_list)):
                ...
                merged_layers = [node_layers[i] for i in members[a]] + [
                    node_layers[i] for i in members[b]
                ]
        ...
        logger.debug(
            "Merging cluster index %d into %d. Remaining clusters: %d",
            best_b,
            best_a,
            len(active) - 1,
        )
...
def clusters_to_supernodes(
    ...
    *,
    ...
    for i, sn in enumerate(middle):
        ...
        out.append(
            _supernode_from_member_ids(
                prune_graph, f"{middle_prefix}_{i}", list(sn), k, id_to_idx=id_to_idx
            )
        )
    ...
    for i, sn in enumerate(emb):
        k = _classify_node(sn[0], nodes_by_id)
        emb_logit.append(
            _supernode_from_member_ids(prune_graph, f"SN_EMB_{i}", list(sn), k, id_to_idx=id_to_idx)
        )
    for i, sn in enumerate(logit):
        k = _classify_node(sn[0], nodes_by_id)
        emb_logit.append(
            _supernode_from_member_ids(
                prune_graph, f"SN_LOGIT_{i}", list(sn), k, id_to_idx=id_to_idx
            )
        )
...
def mapping_dict_to_supernodes(
    ...
    for name, feats in mapping.items():
        ...
        out.append(
            _supernode_from_member_ids(prune_graph, name, list(feats), k, id_to_idx=id_to_idx)
        )
    return out
...
def _causal_mass_loss(sng: SummaryGraph) -> float:
    ...
    internal_mag = 0.0
    for idxs in _feature_member_index_lists(sng):
        if len(idxs) >= 2:
            internal_mag += float(np.abs(adj[np.ix_(idxs, idxs)]).sum())
    return internal_mag / total_mag


def _external_mass_loss(sng: SummaryGraph, causal_loss: float) -> float:
    ...
    return max(total_loss - causal_loss, 0.0)


def _dag_loss(sng: SummaryGraph) -> float:
    index_lists = [[n.node_idx for n in sn.features if n.node_idx >= 0] for sn in sng.supernodes]
    super_adj = compute_sn_adj(index_lists, sng.pruned_adj)
    total_mag = float(np.abs(super_adj).sum())
    if total_mag <= EPS:
        return 0.0
    ...
    for target in range(n):
        for source in range(n):
            if super_adj[target, source] != 0.0 and rank[source] > rank[target]:
                backward_mag += abs(float(super_adj[target, source]))
    return backward_mag / (total_mag + EPS)


def _middle_labels_from_rows(
    rows: list[Supernode],
    ...
    for row in rows:
        locals_ = [
            middle_id_to_local[node.node_id]
            for node in row.features
            if node.node_id in middle_id_to_local
        ]
        if not locals_:
            continue
...
def _atomicity_loss(cos_middle: np.ndarray, labels: np.ndarray, theta: float = 0.0) -> float:
    if labels.size < 2:
        return 0.0
    i, j = np.triu_indices(labels.size, k=1)
    same = (labels[i] == labels[j]) & (labels[i] >= 0)
    values = cos_middle[i, j][same]
    return float((theta - values).sum())
...
def _partition_score(
    *,
    ...
    labels = _middle_labels_from_rows(rows, middle_id_to_local, role_vectors_middle.shape[0])
    cos_middle = _cosine_similarity(role_vectors_middle, nonnegative=False)
    atomicity_loss = _atomicity_loss(cos_middle, labels)
    causal_loss = _causal_mass_loss(sng)
    external_loss = _external_mass_loss(sng, causal_loss)
    dag_loss = _dag_loss(sng)
    return {
        "L": float(atomicity_loss + lambda_causal * (causal_loss + external_loss)),
        "L_atom": float(atomicity_loss),
        "C_causal": float(causal_loss),
...
def find_best_k(
    ...
    for k in range(k_min, k_max + 1):
        ...
        sc["L"] = float(sc["L"]) + float(prune_loss)
...
def find_best_k_for_clusterer(
    *,
    ...
    if n_middle < 3:
        ...
        result["L"] = float(result["L"]) + float(prune_loss)
    ...
    *,
    ...
    if num_clusters == "auto":
        if method == "spectral":
            ...
            k, _ = find_best_k_for_clusterer(
                prune_graph=prune_graph, clusterer=_agg, lambda_causal=lambda_causal
            )
    else:
        k = int(num_clusters)
