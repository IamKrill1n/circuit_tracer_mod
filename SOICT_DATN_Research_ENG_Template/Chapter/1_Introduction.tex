...
neurons in the graph and uses spectral clustering to cluster them. This approach does not take structural information into account,
making the clustering less mechanistically faithful. No existing method produces a compact and faithful structural summary
...
visualizing --- that produces a concise summary graph that is faithful to the original attribution graph.
...
problem and detailing its three stages of pruning, clustering, and visualizing. Chapter~4
evaluates the method empirically, defining the faithfulness metrics, reporting the pruning
and clustering comparisons, and presenting the steering-intervention study that validates
the summary graphs against the underlying model. Chapter~5 discusses the findings and
their limitations and concludes with directions for future work.
