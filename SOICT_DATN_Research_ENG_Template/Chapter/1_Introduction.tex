...
al.~\cite{ameisen2025circuit} prune it by hand, keeping the nodes and edges that account
for most of the influence on the output and discarding the rest, and then group the
surviving features into \emph{supernodes} by inspection, collecting features that play the
...
ADAG~\cite{arora2026adag} uses a language model to describe an attribution graph in natural
language, removing the labelling bottleneck, but it describes a graph whose structure is
taken as given rather than compressing that structure. Work on circuits in the neuron
...
grouping methods. General graph-summarization objectives, such as causal-DAG
summarization~\cite{youngmann2025causal}, compress large directed graphs but assume
unweighted, variable-level causal graphs and carry no notion of faithfulness to an
underlying network. No existing method produces a compact and faithful structural summary
...
interpreting --- that solves the problem, including an exact integer-program clusterer for
small graphs and scalable spectral and agglomerative clusterers for larger ones.
...
and that the supernodes behave as the single mechanisms their labels claim.
\end{itemize}
