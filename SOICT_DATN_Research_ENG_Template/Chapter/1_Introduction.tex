...
% Draft:
% Mechanistic Interpretability -> Circuit Tracing -> SAE -> Attribution Graph -> The need for automated summary

Large language models (LLMs) have achieved remarkable success in various natural language processing tasks, but their inner workings remain largely opaque. Mechanistic interpretability aims to reverse engineer the interal mechanism of the neural network down to human-interpretable algorithm. One important goal of Mech interp is circuit tracing, which identifies specific pathways of activation within the model that correspond to particular linguistic phenomena. Many approaches compete to find the right building block for circuit tracing, namely neurons, attention heads. \cite{circuit_tracer} propose a novel method to use cross-layer transcoder as the replacement model to extract an attribution graph, describing the steps the model make to output the next token for a given input prompt. However, the attribution graph is often too large and complex, and studing them often requires manual effort which could take hours, or simply infisible for long prompts. Therefore, there is a need for an automated method to summarize the attribution graph, which can help researchers understand the model's behavior in a more efficient and effective way.
...
This thesis aims to develop an automated method for summarizing attribution graphs, which can help researchers understand the model's behavior in a more efficient and effective way. To achieve this goal, we propose a novel method that leverages the structure of the attribution graph to identify key pathways of activation and generate concise summaries that capture the most important information. Our method is designed to be flexible and adaptable, allowing it to be applied to a wide range of models and tasks.
...
% - We formulate the problem, and propose a novel method for solving it.
% - We evaluate our method on well-known circuits, and show that it outperforms simple baselines.
% - We show that our method produce steerable summaries, which can be used to understand the model's behavior in different ways.

Our contributions are as follows:
- We formulate the problem of summarizing attribution graphs, and propose a novel method for solving it.
- We evaluate our method on well-known circuits, and show that it outperforms simple baselines.
- We show that our method produce steerable summaries, which can be used to understand the model's behavior in different ways.
