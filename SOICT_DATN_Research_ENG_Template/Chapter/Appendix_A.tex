\documentclass[../main.tex]{subfiles}
\begin{document}

\section{Shap values calculation}
\label{app:shap}

The token weight vector $\tau$ in Section~\ref{sec:problem} is initialized from
per-token SHAP attributions computed on the same causal language model that produced
the attribution graph. The implementation follows the high-level SHAP language-model
interface: a \texttt{Text} masker perturbs input token segments, and
\texttt{TeacherForcing} scores each masked prompt by the log-odds of producing a
fixed continuation $Y$.

\paragraph{Explained target.}
By default, $Y$ is the one-token continuation sampled from
\texttt{model.generate} on the unmasked prompt.
When the graph specifies a target logit, $Y$ is the single vocabulary token
decoded from that logit id, so the attribution aligns with the graph's prediction
target rather than a fresh sample.

\paragraph{Masking.}
Input segments are masked with the placeholder \texttt{...} and collapsed into one
mask token (\texttt{collapse\_mask\_token=True}). A leading BOS token is removed
from the SHAP input when present, then zero-padded back into the final weight
vector so lengths match \texttt{prompt\_tokens}. For chat-formatted prompts,
\texttt{pin\_special\_tokens=True} replaces the default masker with a variant that
marks tokenizer special and added tokens as invariant, so scaffold tokens such as
\texttt{<|im\_start|>} are never perturbed. Optionally,
\texttt{masker\_keep\_prefix=$k$} leaves the first $k$ segments fixed during masking.

\paragraph{From raw SHAP values to $\tau$.}
The explainer returns a matrix of shape (input segments $\times$ output tokens).
Contributions are summed over the output axis (with a transpose check when the
backend returns segment counts along the wrong axis) to obtain one scalar per input
segment. Special tokens matching \texttt{<...>} and any prefix-pinned positions
receive zero mass after normalization.

The raw scores are mapped to a probability vector over non-special prompt tokens
via one of four normalizers: softmax, sparsemax, entmax-1.5, or entmax with
$\alpha \in (1, 2]$ (default $\alpha = 1.25$). Special-token positions are masked
before normalization and forced to zero afterward; remaining mass is renormalized
to sum to one. The graph entry point
\texttt{get\_token\_attribution\_from\_graph} reads \texttt{prompt},
\texttt{prompt\_tokens}, and the target-logit feature id from graph metadata, then
returns this normalized vector for use as $\tau$.

\section{ILP Pareto curve}

We trace the Pareto curve of the ILP solver with theta = 0.65. The x-axis is the causal epsilon, and the y-axis is the atomicity objective.


\begin{figure}[ht]
\centering
\includegraphics[width=0.75\linewidth]{Figure/theta_p65_average_pareto.png}
\caption{ILP Pareto curve with theta = 0.65}
\label{fig:ILP_Pareto_Curve}
\end{figure}

\section{Labeling prompt}
The following system prompt is provided to an LLM to label the supernodes of a
summary graph.

\begin{lstlisting}[style=prompt,caption={...},label={lst:label-prompt}]
You are explaining a summary graph of a language model's computation.

You will be given:

- The prompt processed by the model.
- The token predicted by the model.
- A set of supernodes.
- A set of features belonging to each supernode.

Each feature is described only by:
- Layer
- Position
- <MAX_ACTIVATING_TOKENS>
- <TOKENS_AFTER_MAX_ACTIVATING_TOKEN>
- <TOP_POSITIVE_LOGITS>
- <TOP_ACTIVATING_TEXTS>

Your goal is NOT to explain what the features do in general.

Your goal is to explain what information each supernode contributes to THIS computation.

Always interpret the supernodes in the context of:

1. The prompt.
2. The predicted token.
3. The layer and position evidence for each feature.

--------------------------------------------------
ROLE DEFINITIONS
--------------------------------------------------

Input:
- Activates on a token directly present in the prompt.
- Recognizes entities, words, phrases, syntax, or local patterns.
- Extracts evidence from the prompt.
- Usually appears early in the computation.

Abstract:
- Represents concepts, relations, themes, reasoning steps, or intermediate computations.
- Combines or transforms information from other supernodes.
- Usually appears in the middle of the computation.

Output:
- Supports prediction of the next token.
- Promotes candidate outputs.
- Often reflected in top positive logits.
- Usually appears near the end of the computation.

Trash:
- No consistent pattern exists.
- Evidence is noisy, unrelated to the computation.
- Use only when no meaningful interpretation can be supported.

--------------------------------------------------
GLOBAL ANALYSIS PROCEDURE
--------------------------------------------------

1. Read the prompt and predicted token carefully.

2. Examine ALL supernodes before labeling any individual supernode.

3. Identify major information sources in the prompt.

4. Identify major concepts required to predict the next token.

5. For each supernode:
   - Inspect its features.
   - Inspect each feature's layer and position.
   - Determine what information it contributes to this computation.

6. Prefer explanations that make the set of supernodes coherent as a whole.

--------------------------------------------------
LABELING GUIDELINES
--------------------------------------------------

Labels should represent information carried by the supernode.

Good labels:
- France
- Capital cities
- Geographic entities
- Country-capital relation
- Numbers ~30
- say Paris
- say J_

Bad labels:
- Neuron that activates on...
- This cluster seems related to...
- General semantic feature

Prefer:
- Specific concepts over broad themes.
- Labels that distinguish related supernodes.
- Information relevant to the prompt.
- Computation-relevant explanations.
- Roles in this computation rather than global neuron behavior.

--------------------------------------------------
CONSISTENCY RULES
--------------------------------------------------

When multiple supernodes participate in the same reasoning chain:

Input:
    France

Abstract:
    Country-capital relation

Output:
    Say Paris

Adjacent supernodes should typically represent different stages of computation unless the evidence strongly suggests otherwise.

--------------------------------------------------
OUTPUT FORMAT
--------------------------------------------------

Return JSON only.

{
  "supernodes": [
    {
      "id": "<supernode id>",
      "role": "Input | Abstract | Output | Trash",
      "label": "<1-5 word description>",
      "description": "<one short sentence describing the contribution to the prediction>"
    }
  ]
}
\end{lstlisting}

\section{Multihop results}

We present some summary graphs from the multihop dataset.

% \section{General Regulations}
% Dưới đây là một số quy định và hướng dẫn viết đồ án tốt nghiệp mà bắt buộc sinh viên phải đọc kỹ và tuân thủ nghiêm ngặt.

% Sinh viên cần đảm bảo tính thống nhất toàn báo cáo (font chữ, căn dòng hai bên, hình ảnh, bảng, margin trang, đánh số trang, v.v.). Để làm được như vậy, sinh viên chỉ cần sử dụng các định dạng theo đúng template ĐATN này. Khi paste nội dung văn bản từ tài liệu khác của mình, sinh viên cần chọn kiểu Copy là “Text Only” để định dạng văn bản của template không bị phá vỡ/vi phạm.

% Tuyệt đối cấm sinh viên đạo văn. Sinh viên cần ghi rõ nguồn cho tất cả những gì không tự mình viết/vẽ lên, bao gồm các câu trích dẫn, các hình ảnh, bảng biểu, v.v. Khi bị phát hiện, sinh viên sẽ không được phép bảo vệ ĐATN.

% Tất cả các hình vẽ, bảng biểu, công thức, và tài liệu tham khảo trong ĐATN nhất thiết phải được SV giải thích và tham chiếu tới ít nhất một lần. Không chấp nhận các trường hợp sinh viên đưa ra hình ảnh, bảng biểu tùy hứng và không có lời mô tả/giải thích nào.

% Sinh viên tuyệt đối không trình bày ĐATN theo kiểu viết ý hoặc gạch đầu dòng. ĐATN không phải là một slide thuyết trình; khi người đọc không hiểu sẽ không có ai giải thích hộ. Sinh viên cần viết thành các đoạn văn và phân tích, diễn giải đầy đủ, rõ ràng. Câu văn cần đúng ngữ pháp, đầy đủ chủ ngữ, vị ngữ và các thành phần câu.
% Khi thực sự cần liệt kê, sinh viên nên liệt kê theo phong cách khoa học với các ký tự La Mã. Ví dụ, nhiều sinh viên luôn cảm thấy hối hận vì (i) chưa cố gắng hết mình, (ii) chưa sắp xếp thời gian học/chơi một cách hợp lý, (iii) chưa tìm được người yêu để chia sẻ quãng đời sinh viên vất vả, và (iv) viết ĐATN một cách cẩu thả.

% Trong một số trường hợp nhất thiết phải dùng các bullet để liệt kê, sinh viên cần thống nhất Style cho toàn bộ các bullet các cấp mà mình sử dụng đến trong báo cáo. Nếu dùng bullet cấp 1 là hình tròn đen, toàn bộ báo cáo cần thống nhất cách dùng như vậy; ví dụ như sau:

% \begin{itemize}
% \item Đây là mục 1 – Thực sự không còn cách nào khác tôi mới dùng đến việc bullet trong báo cáo.
% \item Đây là mục 2 – Nghĩ lại thì tôi có thể không cần dùng bullet cũng được. Nên tôi sẽ xóa bullet và tổ chức lại hai mục này trong báo cáo của mình cho khoa học hơn. Tôi muốn thầy cô và người đọc cảm nhận được tâm huyết của tôi trong từng trang báo cáo ĐATN.
% \end{itemize}


% \section{Majoring}
% Sinh viên lưu ý viết đúng ngành/chuyên ngành trên bìa và trên gáy theo đúng quy định của Trường. Ngành học hay chuyên ngành học phụ thuộc vào ngành học mà sinh viên đăng ký. Sinh viên có thể đăng nhập trên trang quản lý học tập của mình để xem lại chính xác ngành học của mình. 

% Một số ví dụ sinh viên có thể tham khảo dưới đây, trong trường hợp có chuyên ngành thì sinh viên không cần ghi chuyên ngành:

% \begin{outline}
%  \1 Đối với kỹ sư chính quy:
%    \2 Từ K61 trở về trước: Ngành Kỹ thuật phần mềm
%    \2 Từ K62 trở về sau: Ngành Khoa học máy tính
%  \1 Đối với cử nhân:
%    \2 Ngành Công nghệ thông tin
%  \1 Đối với chương trình EliteTech:
%    \2 Chương trình Việt Nhật/KSTN: Ngành Công nghệ thông tin
%    \2 Chương trình ICT Global: Ngành Information Technology
%    \2 Chương trình DS\&AI: Ngành Khoa học dữ liệu
% \end{outline}

% \section{Bulleting and Numbering}
% Việc sử dụng danh sách trong LaTeX khá đơn giản và không yêu cầu sinh viên phải thêm bất kỳ gói bổ sung nào. LaTeX cung cấp hai môi trường liệt kê đó là:
% \begin{itemize}
% \item Đánh dấu (bullet) là kiểu liệt kê không có thứ tự. Để sử dụng kiểu liệt kê đánh dấu, chúng ta khai báo như sau\\ 
% \verb$\begin{itemize}$\\
%  \verb$\item$ Nội dung thứ nhất được viết ở đây.\\
% \verb$\item$ Nội dung thứ hai được viết ở đây.\\
%  \verb$\item$ ...\\
% \verb$\end{itemize}$
% \item Đánh số (numering) là kiểu liệt kê có thứ tự. Để sử dụng kiểu liệt kê đánh số, chúng ta khai báo như sau\\
% \verb$\begin{enumerate}$\\
%  \verb$\item$ Nội dung thứ nhất được viết ở đây.\\
% \verb$\item$ Nội dung thứ hai được viết ở đây.\\
%  \verb$\item$ ...\\
% \verb$\end{enumerate}$
% \end{itemize}
% Chú ý các nội dung trình bày trong cả hai môi trường liệt kê theo sau lệnh \verb$\item$. Ngoài ra LaTeX còn cung cấp một số kiểu liệt kê khác, sinh viên có thể tham khảo tại \url{https://www.overleaf.com/learn/latex/Lists}
% \section{Table insertion}
% \begin{table}[h!]
% \centering
% \begin{tabular}{||c c c c||} 
%  \hline
%  Col1 & Col2 & Col2 & Col3 \\ [0.5ex] 
%  \hline\hline
%  1 & 6 & 87837 & 787 \\ 
%  2 & 7 & 78 & 5415 \\
%  3 & 545 & 778 & 7507 \\
%  4 & 545 & 18744 & 7560 \\
%  5 & 88 & 788 & 6344 \\ [1ex] 
%  \hline
% \end{tabular}
% \caption{Table to test captions and labels.}
% \label{table:1}
% \end{table}
% Bảng \ref{table:1} là ví dụ về cách tạo bảng. Tất cả các bảng biểu phải được đề cập đến trong phần nội dung và phải được phân tích và bình luận.  Chú ý: Tạo bảng trong Latex khá phức tạp và mất thời gian, vì vậy sinh viên có thể sử dụng các công cụ hỗ trợ tạo bảng (Ví dụ: \url{https://www.tablesgenerator.com/}).
% Sinh viên có thể tìm hiểu sâu hơn về cách chèn ảnh trong Latex tại link \url{https://www.overleaf.com/learn/latex/Tables}.

% \section{Figure Insertion}
% \begin{figure}
% \centering
% \includegraphics[width=0.75\linewidth]{Figure/IoT.png}
% \caption{Internet vạn vật}
% \label{fig:iot}
% \end{figure}

% Hình \ref{fig:iot} là ví dụ về cách chèn ảnh. Lưu ý chú thích của hình vẽ được đặt ngay dưới hình vẽ. Sinh viên có thể tìm hiểu sâu hơn về cách chèn ảnh trong Latex tại \url{https://www.overleaf.com/learn/latex/Inserting_Images}.

% Chú ý, tất cả các hình vẽ phải được đề cập đến trong phần nội dung và phải được phân tích và bình luận. 

% \section{Reference}
% \paragraph{Listing}\mbox{}

% Áp dụng cách liệt kê theo quy định của IEEE. Ví dụ của việc trích dẫn như sau \cite{ameisen2025circuit}. Cụ thể, sinh viên sử dụng lệnh \verb!\cite{}! như sau \cite{vaswani2017attention}. Chỉ những tài liệu được trích dẫn thì mới xuất hiện trong phần Tài liệu tham khảo. Tài liệu tham khảo cần có nguồn gốc rõ ràng và phải từ nguồn đáng tin cậy. Hạn chế trích dẫn tài liệu tham khảo từ các website, từ wikipedia.
% \paragraph{Types of Reference}\mbox{}

% Các nguồn tài liệu tham khảo chính là sách, bài báo trong các tạp chí, bài báo trong các hội nghị khoa học và các tài liệu tham khảo khác trên internet.

% \section{Equations}
% Các gói amsmath, amssymb, amsfonts hỗ trợ viết phương trình/công thức toán học đã được bổ sung sẵn ở phần đầu của file main.tex. Một ví dụ về tạo phương trình \eqref{pt31} như sau 
% \begin{equation}\label{pt31}
%     F(x) = \int^a_b \frac{1}{3}x^3
% \end{equation}
% Phương trình \ref{pt31} là ví dụ về phương trình tích phân. Một phương trình khác không được đánh số thứ tự (gán nhãn)
% \begin{equation*}\label{pt32}
%     x[t_n] = \frac{1}{\sqrt{N}} \sum_{k=0}^{N-1}X[f_k]e^{j 2\pi n k/N}
% \end{equation*}
% Phương trình này thể hiện phép biến đổi Fourier rời rạc ngược (IDFT).

% \section{Qui cách đóng quyển}
% Phần bìa trước chế bản theo qui định; bìa trước và bìa sau là giấy liền khổ. Sử dụng keo nhiệt để dán gáy khi đóng quyển thay vì sử dụng băng dính và dập ghim như mô tả ở Hình~\ref{fig:Bia}
% \begin{figure}
% \centering
% \includegraphics[width=0.5\linewidth]{Figure/Bia.PNG}
% \caption{Qui cách đóng quyển đồ án}
% \label{fig:Bia}
% \end{figure} 
% Phần gáy ĐATN cần ghi các thông tin tóm tắt sau:
% Kỳ làm ĐATN - Ngành đào tạo - Họ và tên sinh viên - Mã số sinh viên. Ví dụ:
% \begin{quote}
% 2022.1 - KỸ THUẬT MÁY TÍNH - NGUYỄN VĂN A - 20221234
% \end{quote}
% Qui cách ghi chữ phần gáy như hình dưới đây:
% \begin{figure}
% \centering
% \includegraphics[width=0.5\linewidth]{Figure/GayBia.PNG}
% \caption{Qui cách đóng quyển đồ án}
% \label{fig:Bia}
% \end{figure} 



\end{document}