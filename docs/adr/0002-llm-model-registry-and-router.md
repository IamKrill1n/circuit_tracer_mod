...
The labeling code must work with an unknown, changing set of models: official OpenAI, Gemini,
...
(`openai`, `gemini`, `openai_compat`), owning retry/backoff internally. Generation settings
...
- `thinking_effort` is a unified enum (`low`/`medium`/`high`/`None`) translated per provider
  (OpenAI `reasoning_effort`; Gemini `thinking_budget` token count).
